...
## 零手动：自动启动 + 自动开预览（强烈推荐）
> 仓库已内置 `.vscode/preview.yml`：打开工作空间即**自动执行** `start-cloudstudio.sh`，并自动识别 8003 端口、自动弹出预览/公网链接。你**只需打开工作空间**，无需手动开终端敲命令，也无需点端口卡片。

- 该文件已随仓库提交，Cloud Studio 从 Gitee 克隆或 `git pull` 后即可生效。
- 若打开后没自动启动：左侧「运行」下拉 →「查看配置文件」确认已读取 `preview.yml`；或直接开终端执行 `bash start-cloudstudio.sh`（前台）兜底。
- 本地改完代码推 Gitee 后，Cloud Studio 终端里 `git pull` 拉最新，端口插件通常会自动重启服务；若未自动重启，停掉终端里的 uvicorn 再 `bash start-cloudstudio.sh` 即可。
...
3. 把链接发给别人即可打开；用 **`admin` / `baofu123`** 登录
...
- 你用 `admin / baofu123` 登录，可在设置里管理/禁用其他账号

## 本地数据迁移到云端（首次必做）
云端工作空间是**全新空库**，你本地已经加好的「可投池 / 规则 / 标签」不会自动同步，需要手动搬一次：
1. 本地：打开 `http://127.0.0.1:8003` → 登录 →「设置 / 数据迁移」→「导出全部」→ 下载得到一个 JSON。
2. 云端：用 `admin / baofu123` 登录云链接 →「设置 / 数据迁移」→「导入全部」→ 选刚才的 JSON → 确认。
3. 导入后本地自选池、规则、标签全部出现在云端；之后云端数据以云端为准（本地与云端相互独立）。

> 提示：以后换电脑/重装，只要从云端「导出全部」再「导入」即可，不依赖本地文件。

## 休眠 / 重置 与数据防丢
- **休眠（停止工作空间）**：磁盘保留，重新打开工作空间 + `preview.yml` 自动启动后，数据**原样恢复**，无需重新导入。
- **重置（工作空间被系统回收/重置）**：磁盘清空，SQLite 数据（账号/自选池/规则）全没。
  - 系统已内置**每日 15:35 自动整库备份**到服务器 `data/backups/`（保留 14 份），admin 在设置页可一键「整库恢复」。
  - 强烈建议再开 **腾讯云 COS 云端备份**（见下方章节）：环境被重置也不丢，新环境一键还原所有账号。
  - 平时也可随时「设置 / 数据迁移 → 导出全部」存一份 JSON 到本地做兜底。
...
### 6. 灾难恢复（环境被重置时）
1. 重新 `bash start-cloudstudio.sh`（会自动初始化新库 + admin/baofu123）
